...
public:
    ...
    //移动构造函数与移动赋值运算符函数优先于拷贝构造函数与赋值运算符函数的执行
    
    //移动构造函数与移动赋值运算符函数编译器不会自动生成，必须手写

    
    //String s3 = String("world");
    //String s3 = s1;
    //移动构造函数
    String(String &&rhs) // 只识别右值  比如world  浅拷贝就行了
    : _pstr(rhs._pstr)  // （右值）临时对象的空间用起来了  不销毁右值空间
    {
    ...
    //s4 = std::move(s4)  //将左值变为右值
    //s5 = std::move(s4)
    String &operator=(String &&rhs)  // 识别右值  
    {
...
void test()
{
    ...
    cout << endl;
    String s3 = "world";  //隐式转换S tring("world")
    cout << "s3 = " << s3 << endl;
